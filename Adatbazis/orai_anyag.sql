...
WHERE szuletesi_ev < 1980 AND (stilus = "rock" OR stilus = "metal")  -- stilus IN ('rock', 'metal')
...
SELECT muvesznev, a.cim AS album_cim, a.kiadas_eve, COUNT(d.id) AS dalok_szama, AVG(d.hossz_mp) AS atlag_hossz_mp FROM eloadok e
INNER JOIN albumok a ON a.eloadoid = e.id
INNER JOIN dalok d ON d.albumid = a.id
